...
由题可知，一个数据报长度为 4000B, 而首部长度是固定首部长度即为20B, 所以：一个数据报 = 首部 + 数据部分, 因此数据部分为 4000 - 20 = 3980 B

题中所说的「此网络能够传送的最大数据长度为 1500 字节」是指分片之后的数据报即 首部+数据部分 不能超过1500B, 所以需要**考虑首部长度**

第1个数据报片：数据部分 = 1480 , 3980 - 1480 = 2500 > 1500 , 后面还有分片，故 MF = 1 , 片偏移字段 = 0 / 8 = 0

第2个数据报片：数据部分 = 1480 , 2500 - 1480 = 1020 < 1500 , 后面还有分片，故 MF = 1 , 片偏移字段 = 1480 / 8 = 185

第3个数据报片：数据部分 = 1020 , MF = 0 , 片偏移字段 = (1480 + 1480) / 8 = 370
